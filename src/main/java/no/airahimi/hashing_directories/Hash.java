...
public class Hash {
	...
	private void writeToHashFile(File fileToHash) {
		...
		try {
			...
			dos.writeBytes(fileToHash.getAbsolutePath() + "," + hash_sha256 + Main.NEW_LINE);
			//System.out.println(fileToHash + " has been hashed");
			dos.close();

			output.append(fileToHash + " has been hashed" + Main.NEW_LINE);
	...
	private void clearFile() {
		try {
			...
			dos.writeBytes("########## HASH FILE ##########" + Main.NEW_LINE);
			dos.close();
		} catch (FileNotFoundException e) {
